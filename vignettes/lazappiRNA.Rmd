...
filtering could theoretically be applied. For the rest of vignette filtering
will be applied when suggested by the authors of _HTSFilter_. Unfortunately this
is different for each method and can't be automated. For example for _voom_ no
methods for normalised datasets exist so the most appropriate stage is on the
raw counts.

```{r filtering}
filtered <- HTSFilter(objects$voom)
objects$voom <- filtered$filteredData
```
...
```{r venn}
venn <- vennDiagram(tested, alpha = 0.05)
